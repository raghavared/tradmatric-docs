...
**For Tradmatrix**: Fair winner selection from distributed tickets without any party having influence over the outcome.
...
let winning_ticket = (var.value % total_tickets) + 1;
...
```
User 1: init_user + buy_ticket (BATCHED)
User 2-N: buy_ticket (SEQUENTIAL)
...
```
TX: pick_winner
  ├─ Calculates: winning_ticket = (final_value % total) + 1
...
## Ticket Numbering & Winner Selection

### 1-Based Indexing

Tickets are numbered 1 to N (user-friendly):

```rust
// Asset created with numberOfTickets = 100
asset.ticketsSold = 0

// Each ticket purchase:
let ticket_number = asset.ticketsSold + 1;  // 1, 2, 3, ..., 100
asset.ticketsSold += 1;

// When sold out:
asset.ticketsSold == 100 (all tickets sold)
```
...
// Select winning ticket (1-based)
let winning_ticket_number = (random_u64 % asset.ticketsSold) + 1;
```
