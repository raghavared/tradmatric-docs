...
<Steps>
  <Step title="User Selects Asset">
    Browse and choose asset to enter a draw
  </Step>
  ...
  <Step title="Token Issued">
    User receives token, no wallet needed
  </Step>
</Steps>
...
sequenceDiagram
    ...
    Backend->>Solana: Buy token on-chain
    Solana-->>Backend: Token created
...
5. Execute on-chain token purchase
...
User Pays:           $104.90
Asset Owner Gets:    $98.50 (from token pool)
```
...
- **Average token size**: $150+ (higher than crypto users)
